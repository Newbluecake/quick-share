...
|----|--------|----------|--------|----------|------|
| F-001 | 自动检测局域网IP | 1. 执行 `quick-share test.txt` 2. 验证输出包含 192.168.x.x 或 10.x.x.x 格式的IP | P0 | T-004 | ✅ |
| F-002 | 多网卡环境正确选择IP | 1. 在有以太网和WiFi的设备上执行 2. 验证优先选择以太网IP | P0 | T-004 | ✅ |
| F-003 | 自动选择可用端口 | 1. 执行 `quick-share test.txt` 2. 验证默认使用8000端口 3. 占用8000后再次执行，验证使用8001 | P0 | T-006 | ✅ |
| F-004 | 仅允许下载指定文件 | 1. 启动服务分享 test.txt 2. 尝试访问 /etc/passwd 3. 验证返回403或404 | P0 | T-005 | ✅ |
| F-005 | 路径遍历攻击防护 | 1. 启动服务 2. 尝试访问 `/../etc/passwd` 3. 验证被拦截 | P0 | T-005 | ✅ |
| F-006 | 显示curl下载命令 | 1. 执行 `quick-share test.txt` 2. 验证输出包含 `curl http://...` 命令 | P0 | T-003 | ✅ |
| F-007 | 显示wget下载命令 | 1. 执行 `quick-share test.txt` 2. 验证输出包含 `wget http://...` 命令 | P0 | T-003 | ✅ |
| F-008 | curl命令可直接使用 | 1. 复制输出的curl命令 2. 在另一台机器执行 3. 验证文件下载成功 | P0 | T-009 | ✅ |
| F-009 | 浏览器可访问下载 | 1. 复制HTTP URL 2. 在浏览器打开 3. 验证触发文件下载 | P0 | T-009 | ✅ |
| F-010 | 默认下载次数10次 | 1. 执行 `quick-share test.txt` 2. 下载10次 3. 验证第11次时服务已停止 | P1 | T-006 | ✅ |
| F-011 | 自定义下载次数 | 1. 执行 `quick-share test.txt -n 3` 2. 下载3次 3. 验证服务自动停止 | P1 | T-006 | ✅ |
| F-012 | 默认5分钟自动停止 | 1. 执行 `quick-share test.txt` 2. 等待5分钟 3. 验证服务自动停止 | P1 | T-006 | ✅ |
| F-013 | 自定义运行时长 | 1. 执行 `quick-share test.txt -t 1m` 2. 等待1分钟 3. 验证服务自动停止 | P1 | T-006 | ✅ |
| F-014 | 禁用时长限制 | 1. 执行 `quick-share test.txt -t 0` 2. 验证服务持续运行（需手动停止） | P1 | T-006 | ✅ |
| F-015 | 显示下载日志 | 1. 启动服务 2. 另一台机器下载文件 3. 验证控制台显示时间戳、IP、状态码 | P1 | T-003 | ✅ |
| F-016 | 显示下载进度 | 1. 启动服务 `-n 3` 2. 每次下载后验证显示 "1/3", "2/3", "3/3" | P1 | T-006 | ✅ |
| F-017 | 自定义端口 | 1. 执行 `quick-share test.txt -p 9000` 2. 验证使用9000端口 | P1 | T-007 | ✅ |
| F-018 | 帮助信息显示 | 1. 执行 `quick-share --help` 2. 验证显示所有可用参数 | P1 | T-007 | ✅ |
| F-019 | 边缘场景：文件不存在 | 1. 执行 `quick-share nonexistent.txt` 2. 验证显示错误信息并退出 | P1 | T-008 | ✅ |
| F-020 | 边缘场景：文件路径为目录 | 1. 执行 `quick-share /home/user/` 2. 验证显示错误信息（不支持目录） | P1 | T-008 | ✅ |
| F-021 | 边缘场景：无参数执行 | 1. 执行 `quick-share` 2. 验证显示使用说明 | P2 | T-007 | ✅ |
| F-022 | 边缘场景：Ctrl+C优雅退出 | 1. 启动服务 2. 按 Ctrl+C 3. 验证服务优雅关闭 | P2 | T-008 | ✅ |
| F-023 | 显示文件大小（人类可读） | 1. 分享一个 1.5MB 文件 2. 验证输出显示 "1.5 MB" 而非字节数 | P2 | T-002 | ✅ |
